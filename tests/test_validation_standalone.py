...
def test_parameter_validation_logic():
    ...
    def validate_switch_point_conflict(lightning_start, switch_step):
        if lightning_start > switch_step:
            raise ValueError(f"lightning_start ({lightning_start}) cannot be greater than switch_step ({switch_step}). Either decrease lightning_start or increase switch_step, or use a different switching strategy.")
...
def test_edge_case_combinations():
    ...
    def validate_all_parameters(lightning_steps, lightning_start, base_steps, switch_strategy, switch_step):
        ...
        if switch_strategy == "Manual switch step" and switch_step != -1:
            if switch_step < 0:
                raise ValueError(f"switch_step ({switch_step}) must be >= 0")
            if switch_step >= lightning_steps:
                raise ValueError(f"switch_step ({switch_step}) must be < lightning_steps ({lightning_steps})")
            if switch_step < lightning_start:
                raise ValueError(f"switch_step ({switch_step}) cannot be less than lightning_start ({lightning_start}). The high-noise model needs at least some steps before switching. If you want low-noise only, set lightning_start=0 as well.")
